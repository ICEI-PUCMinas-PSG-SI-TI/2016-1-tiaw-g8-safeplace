...
// ─── DADOS (localStorage via db) ──────────────────────────
async function fetchAll() {
  allCompromissos = await db.get('compromissos');
  return allCompromissos;
}

async function fetchByDate(dateStr) {
  return await db.get('compromissos', { data: dateStr });
}

async function createCompromisso(data) {
  return await db.post('compromissos', data);
}

async function updateCompromisso(id, data) {
  return await db.put('compromissos', id, data);
}

async function deleteCompromisso(id) {
  await db.delete('compromissos', id);
}
...
async function openEdit(id) {
  ...
  if (!ev) {
    ev = await db.getById('compromissos', id);
    if (!ev) {
