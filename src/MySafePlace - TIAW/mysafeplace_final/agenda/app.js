const API = 'http://localhost:3000/compromissos';
...
async function fetchAll() {
  try {
    ...
    showToast('⚠️ Servidor não encontrado. Rode: npx json-server db.json');
    return [];
  }
}
...
async function fetchByDate(dateStr) {
  try {
    const r = await fetch(`${API}?data=${dateStr}`);
    return await r.json();
  } catch { return []; }
}
...
async function openEdit(id) {
  ...
  if (!ev) {
    try {
      ev = await fetch(`${API}/${id}`).then(r => r.json());
    } catch {
  ...
  $('form-hora-inicio').value = ev.horaInicio;
  $('form-hora-fim').value = ev.horaFim;
...
async function handleFormSubmit(e) {
  ...
  const payload = {
    ...
    horaInicio: $('form-hora-inicio').value,
    horaFim: $('form-hora-fim').value,
    tipo: $('form-tipo').value,
    participante: $('form-participante').value.trim()
  };

  if (!payload.titulo || !payload.data || !payload.horaInicio || !payload.horaFim || !payload.participante) {
