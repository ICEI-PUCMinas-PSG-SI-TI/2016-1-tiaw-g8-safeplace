document.addEventListener('DOMContentLoaded', function () {
  ...
  // Dados do banco local (localStorage)
  db.get('artigos')
    ...
    .catch(function (erro) {
      carregando.innerHTML = '<i class="bi bi-exclamation-circle text-danger"></i> Erro ao carregar os artigos.';
      console.error(erro);
    });
  ...
  function registrarEventosDeInteracao() {
    document.querySelectorAll('.btn-curtir-lista').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        db.patch('artigos', id, { likes: novoLikes });
    ...
    document.querySelectorAll('.btn-compartilhar-lista').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var id = this.getAttribute('data-id');
        var url = window.location.href.replace(/[^/]*$/, '') + 'artigo.html?id=' + id;
        if (navigator.share) {
